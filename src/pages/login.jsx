...
import { Favourite } from "../fav-mechanics";
...
function Login() {
  ...
  const handleSignIn = async () => {
    try {
      ...
      // After signing in, set user data
      if (!docSnap.exists()) {
  ...
  const setUserData = async (user) => {
    try {
      const db = collection(firestore, "user");
      const userRef = doc(db, user.uid); // Create a reference to the user document using their UID
      const userData = {
        email: user.email,
        balance: { USD: 1000 },
        favourite: [],
